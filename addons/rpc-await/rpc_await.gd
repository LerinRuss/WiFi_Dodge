...
# TODO What if you reach integer overflow of the long run of an server?
var _next_id := 0 
...
#class ConditionalSignal:
	...
	#
	#func _init(top_threshold: int):
		#self.top_threshold = top_threshold
	#
	#func ding():
		#self.counter += 1
		#
		#if self.counter >= self.top_threshold:
			#self.done.emit()

# My Section
func send_rpc_all(callable: Callable) -> Array[Variant]:
	var res: Array[Variant] = []
	
	for peer_id in multiplayer.get_peers():
		var await_res = await send_rpc(peer_id, callable)
		
		res.append(await_res)
